...
def is_valid_walk(walk):
    ...
    for direction in range(0, len(walk)):
        if walk[direction] == 'n': destination[0] += 1
        elif walk[direction] == 's': destination[0] -= 1
        elif walk[direction] == 'e': destination[1] += 1
        elif walk[direction] == 'w': destination[1] -= 1
    if destination == [0,0]: return True
    else: return False 

print(is_valid_walk(['n', 'n', 'e', 'e', 's', 's', 'w', 'w', 'n', 'w']))
